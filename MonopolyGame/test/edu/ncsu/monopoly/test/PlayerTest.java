...
public class PlayerTest extends TestCase {
    ...
    public void testPayRentTo() {
        ...
        gameMaster.btnEndTurnClicked();
        int result2 = gameMaster.getPlayer(0).getMoney();
    ...
    public void testPlayerSetUser() {
        Player p = new Player();
        User u = new User();
        String name = "George";
        u.setName(name);
        p.setUser(u);
        assertEquals(name, p.getUser().getName());

    }

    public void testPurchaseHouseOnePlayer() {
        gameMaster.setNumberOfPlayers(1);
        Player p = gameMaster.getCurrentPlayer();
        Cell c1 = gameMaster.getGameBoard().getCell(1);
        Cell c2 = gameMaster.getGameBoard().getCell(2);
        Cell c3 = gameMaster.getGameBoard().getCell(3);
        p.buyProperty(c1, 0);
        p.buyProperty(c2, 0);
        p.buyProperty(c3, 0);
        gameMaster.movePlayer(p, 3);
        String[] name = p.getMonopolies();
        p.purchaseHouse(name[0], 3);
        assertEquals(1050,p.getMoney());
